...
void lm_minimize(int m_dat, int n_par, double *par, double *err,
...
// *** perform fit.
    ...
    lm_lmdif(m, n, par, err, fvec, control->ftol, control->xtol, control->gtol,
...
void lm_lmdif(int m, int n, double *x, double *err, double *fvec, double ftol,
...
// *** evaluate the function at the starting point and calculate its norm.
    ...
    *info = 0;
    (*evaluate) (x, m, fvec, err, data, info);
...
// O** calculate the jacobian matrix.
	...
	for (j = 0; j < n; j++) {
	    ...
	    x[j] = temp + step;
	    *info = 0;
	    (*evaluate) (x, m, wa4, err, data, info);
...
// OI* evaluate the function at x + p and calculate its norm.
	    ...
	    *info = 0;
	    (*evaluate) (wa2, m, wa4, err, data, info);
...
// *** bring the column of largest norm into the pivot position.
	...
	for (k = j + 1; k < n; k++)
		...
		kmax = k;
	if (kmax == j)
	    goto pivot_ok;	// bug fixed in rel 2.1
